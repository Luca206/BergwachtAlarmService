...
public class LgTvService : ITvService
{
    ...
    public async Task ConnectAsync(string ip, CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogError(e, "Could not connect to LG webOS TV at {Uri}: {Message}", uri, e.Message);
            throw;
        }
        ...
        await SendRawAsync(JsonSerializer.SerializeToUtf8Bytes(registerMsg, _jsonOptions), cancellationToken)
    ...
    public async Task<TResponse?> SendRequestAsync<TResponse>(string uri, object? payload = null,
        CancellationToken cancellationToken = default)
    {
        ...
        await SendRawAsync(JsonSerializer.SerializeToUtf8Bytes(msg, _jsonOptions), cancellationToken)
            .ConfigureAwait(false);
    ...
    private async Task ReceiveLoopAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            while (!cancellationToken.IsCancellationRequested && _ws is { State: WebSocketState.Open })
            {
                ...
                {
                    _logger.LogWarning("WebSocket closed by TV: {CloseStatus} {Description}", _ws.CloseStatus,
